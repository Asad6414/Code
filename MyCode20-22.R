...
data<-read_csv("https://raw.githubusercontent.com/Asad6414/Code/main/data20-22.csv", show_col_types = FALSE)
data$gscpi <- abs(data$gscpi)
# data <- read.csv(file = "data67-99.csv")


################### LEVELS #######################
gdp <- ts(log(data$gdp), start= c(2020, 1), freq = 4) # Correct way to construct the data
real_gdp <- ts(log(data$real_gdp), start= c(2020, 1), freq = 4)
cpi <- ts(log(data$cpi), start= c(2020, 1), freq = 4)
core_cpi <- ts(log(data$core_cpi), start= c(2020, 1), freq = 4)
prate <- ts(data$fedfunds, start= c(2020, 1), freq = 4) # Not necessesary to take log
empration <- ts(data$emratio, start= c(2020, 1), freq = 4) # Not necessesary to take log
wti_oil <- ts(data$wti_oil, start= c(2020, 1), freq = 4) # Not necessesary to take log
gscpi <- ts(log(data$gscpi), start= c(2020, 1), freq = 4)
divisia <- ts(log(data$divisia_m3), start= c(2020, 1), freq = 4)
gov_total <- ts(log(data$gov_total), start= c(2020, 1), freq = 4)
gov_current <- ts(log(data$gov_current), start= c(2020, 1), freq = 4)
total_par <- ts(log(data$total_par), start= c(2020, 1), freq = 4)
total_market <- ts(log(data$total_market), start= c(2020, 1), freq = 4) 
private_par <- ts(log(data$private_par), start= c(2020, 1), freq = 4)
private_market <- ts(log(data$private_market), start= c(2020, 1), freq = 4)
...
info.bv <- VARselect(data.bv, lag.max = 6, type = "const")
info.bv$selection

bv.est <- VAR(data.bv, p = 2, type = "const", season = NULL, exog = NULL)
summary(bv.est)
...
#To load the data and store the time series objects
data<-read_csv("https://raw.githubusercontent.com/Asad6414/Code/main/data_tr20-22.csv", show_col_types = FALSE)
data$gscpi <- abs(data$gscpi)

# gdp_tr <- ts(diff(log(data$gdp)), start= c(2020, 1), freq = 4) # Correct way to construct the data
# real_gdp_tr <- ts(diff(log(data$real_gdp)), start= c(2020, 1), freq = 4)
# cpi_tr <- ts(diff(log(data$cpi)), start= c(2020, 1), freq = 4)
# core_cpi_tr <- ts(diff(log(data$core_cpi)), start= c(2020, 1), freq = 4)
# prate_tr <- ts(data$fedfunds, start= c(2020, 1), freq = 4) # Not necessesary to take to take log and diff
# empration_tr <- ts(data$emratio, start= c(2020, 1), freq = 4) # Not necessesary to take log and diff
# wti_oil_tr <- ts(diff(log(data$wti_oil)), start= c(2020, 1), freq = 4)
# gscpi <- ts(diff(log(data$gscpi)), start= c(2020, 1), freq = 4)
# divisia_tr <- ts(diff(log(data$divisia_m3)), start= c(2020, 1), freq = 4)
# gov_total_tr <- ts(diff(log(data$gov_total)), start= c(2020, 1), freq = 4)
# gov_current_tr <- ts(diff(log(data$gov_current)), start= c(2020, 1), freq = 4)
# total_par_tr <- ts(diff(log(data$total_par)), start= c(2020, 1), freq = 4)
# total_market_tr <- ts(diff(log(data$total_market)), start= c(2020, 1), freq = 4) 
# private_par_tr <- ts(diff(log(data$private_par)), start= c(2020, 1), freq = 4)
# private_market_tr <- ts(diff(log(data$private_market)), start= c(2020, 1), freq = 4)


gdp_tr <- ts(data$gdp, start= c(2020, 1), freq = 4) # Correct way to construct the data
real_gdp_tr <- ts(data$real_gdp, start= c(2020, 1), freq = 4)
cpi_tr <- ts(data$cpi, start= c(2020, 1), freq = 4)
core_cpi_tr <- ts(data$core_cpi, start= c(2020, 1), freq = 4)
prate_tr <- ts(data$fedfunds, start= c(2020, 1), freq = 4) # Not necessesary to take to take log and diff
empration_tr <- ts(data$emratio, start= c(2020, 1), freq = 4) # Not necessesary to take log and diff
wti_oil_tr <- ts(data$wti_oil, start= c(2020, 1), freq = 4)
gscpi_tr <- ts(data$gscpi, start= c(2020, 1), freq = 4)
divisia_tr <- ts(data$divisia_m3, start= c(2020, 1), freq = 4)
gov_total_tr <- ts(data$gov_total, start= c(2020, 1), freq = 4)
gov_current_tr <- ts(data$gov_current, start= c(2020, 1), freq = 4)
total_par_tr <- ts(data$total_par, start= c(2020, 1), freq = 4)
total_market_tr <- ts(data$total_market, start= c(2020, 1), freq = 4) 
private_par_tr <- ts(data$private_par, start= c(2020, 1), freq = 4)
private_market_tr <- ts(data$private_market, start= c(2020, 1), freq = 4)
