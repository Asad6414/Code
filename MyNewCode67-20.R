...
data<-read_csv("https://raw.githubusercontent.com/Asad6414/Code/main/data67-20.csv", show_col_types = FALSE)
# data <- read.csv(file = "data67-99.csv")


################### LEVELS #######################
gdp <- ts(log(data$gdp), start= c(1967, 1), freq = 4) # Correct way to construct the data
real_gdp <- ts(log(data$real_gdp), start= c(1967, 1), freq = 4)
cpi <- ts(log(data$cpi), start= c(1967, 1), freq = 4)
core_cpi <- ts(log(data$core_cpi), start= c(1967, 1), freq = 4)
prate <- ts(data$fedfunds, start= c(1967, 1), freq = 4) # Not necessesary to take log
empration <- ts(data$emratio, start= c(1967, 1), freq = 4) # Not necessesary to take log
wti_oil <- ts(log(data$wti_oil), start= c(1967, 1), freq = 4) # Not necessesary to take log
# gscpi <- ts(log(data$gscpi), start= c(1967, 1), freq = 4)
divisia <- ts(log(data$divisia_m3), start= c(1967, 1), freq = 4)
gov_total <- ts(log(data$gov_total), start= c(1967, 1), freq = 4)
gov_current <- ts(log(data$gov_current), start= c(1967, 1), freq = 4)
total_par <- ts(log(data$total_par), start= c(1967, 1), freq = 4)
total_market <- ts(log(data$total_market), start= c(1967, 1), freq = 4) 
private_par <- ts(log(data$private_par), start= c(1967, 1), freq = 4)
private_market <- ts(log(data$private_market), start= c(1967, 1), freq = 4)

#Plot the data
plot(cbind(gdp, cpi, prate, empration, wti_oil, divisia, gov_total, total_market, private_market))
...
data.bv <- cbind(gdp, cpi, prate, empration, wti_oil, divisia, gov_total, total_market, private_market)
data.bv

colnames(data.bv) <- c("gdp", "cpi", "prate", "empration", "wti_oil", "divisia", "gov_total", "total_market", "private_market")

info.bv <- VARselect(data.bv, lag.max = 12, type = "trend")
info.bv$selection

bv.est <- VAR(data.bv, p = 3, type = "const", season = NULL, exog = NULL)
summary(bv.est)
...
pz.est <- VAR(data.pz, p = 3, type = "const", season = NULL, exog = cbind(t, t2))
summary(pz.est)
...
#To load the data and store the time series objects
data<-read_csv("https://raw.githubusercontent.com/Asad6414/Code/main/data_tr67-20.csv", show_col_types = FALSE)
...
# gscpi <- ts(diff(log(data$gscpi)), start= c(1998, 1), freq = 4)
...
gdp_tr <- ts(data$gdp, start= c(1967, 1), freq = 4) # Correct way to construct the data
real_gdp_tr <- ts(data$real_gdp, start= c(1967, 1), freq = 4) #already transformed in the csv data
cpi_tr <- ts(data$cpi, start= c(1967, 1), freq = 4) #already transformed in the csv data
core_cpi_tr <- ts(data$core_cpi, start= c(1967, 1), freq = 4) #already transformed in the csv data
prate_tr <- ts(data$fedfunds, start= c(1967, 1), freq = 4) # Not necessesary to take to take log and diff
empration_tr <- ts(data$emratio, start= c(1967, 1), freq = 4) # Not necessesary to take log and diff
wti_oil_tr <- ts(data$wti_oil, start= c(1967, 1), freq = 4) #already transformed in the csv data
divisia_tr <- ts(data$divisia_m3, start= c(1967, 1), freq = 4) #already transformed in the csv data
gov_total_tr <- ts(data$gov_total, start= c(1967, 1), freq = 4) #already transformed in the csv data
gov_current_tr <- ts(data$gov_current, start= c(1967, 1), freq = 4) #already transformed in the csv data
total_par_tr <- ts(data$total_par, start= c(1967, 1), freq = 4) #already transformed in the csv data
total_market_tr <- ts(data$total_market, start= c(1967, 1), freq = 4) #already transformed in the csv data
private_par_tr <- ts(data$private_par, start= c(1967, 1), freq = 4) #already transformed in the csv data
private_market_tr <- ts(data$private_market, start= c(1967, 1), freq = 4) #already transformed in the csv data

#Plot the data
plot(cbind(gdp_tr, cpi_tr, prate_tr, empration_tr, wti_oil_tr, divisia_tr, gov_total_tr, total_market_tr, private_market_tr))
...
data_tr.bv <- cbind(gdp_tr, cpi_tr, prate_tr, empration_tr, wti_oil_tr, divisia_tr, gov_total_tr, total_market_tr, private_market_tr)
data_tr.bv

colnames(data_tr.bv) <- c("gdp tr", "cpi tr", "prate tr", "empration tr", "wti_oil tr", "divisia tr", "gov_total tr", "total_market tr", "private_market tr")


info_tr.bv <- VARselect(data_tr.bv, lag.max = 12, type = "trend")
info_tr.bv$selection

bv_tr.est <- VAR(data_tr.bv, p = 3, type = "const", season = NULL, exog = NULL)
summary(bv_tr.est)
...
pz_tr.est <- VAR(data_tr.pz, p = 3, type = "const", season = NULL, exog = cbind(t, t2))
summary(pz_tr.est)
